...
rgb_lable_img = [[0, 0, 0],[64, 0, 128],[64, 64, 0],[0, 128, 192],[0, 0, 192],[128, 128, 128],[64, 64, 128],[192, 128, 128],[192, 64, 0]]
PALETTE = [[0, 0, 0], [1, 1, 1], [2, 2, 2], [3, 3, 3], [4, 4, 4], [5, 5, 5], [6, 6, 6], [7, 7, 7],[8, 8, 8]]
...
def rgb_to_2D_label(_label):  # H W C
    ...
    for idex in range(num_classes):
        ...
        class_mask = np.all(_label == color_mask, axis=-1)  # class_mask将会得到H×W的布尔矩阵
...
class DataLoaderTrain(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #-----------------------------
        tar_seg_img = torch.from_numpy(target_seg).long()    #使用交叉熵损失需要long类型的数据
        # 这个os.path.spilt(path)就自动从path最后一个斜杠切；splitext从最后那个点切开，变成文件名和拓展名
